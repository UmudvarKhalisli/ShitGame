...
function StageRouter() {
  ...
  const [canSeeAdminTrigger, setCanSeeAdminTrigger] = useState(false);
  ...
  useEffect(() => {
    ...
    const hasAdminBootstrap = params.get("admin") === "1";
    const hasAdminSession = window.sessionStorage.getItem("owner_admin_trigger") === "1";

    if (hasAdminBootstrap) {
      window.sessionStorage.setItem("owner_admin_trigger", "1");
    }

    if (hasAdminBootstrap || hasAdminSession) {
      setCanSeeAdminTrigger(true);
    }
